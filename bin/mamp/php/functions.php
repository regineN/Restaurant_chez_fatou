...
if (contains("/Library/", __FILE__)) {
	$configObject->app = "MAMP PRO";

} else {
	$configObject->app = "MAMP";
}
...
function is_cloud_bought() {
    global $configObject;
    if ($configObject->app == "MAMP PRO") {
        return true;
    }
    $i_boughtc = false;
    if (file_exists(dirname(__FILE__) . '/../boughtc')) {
        $i_boughtc = (int) file_get_contents(dirname(__FILE__) . '/../boughtc');
    }
    return $i_boughtc;
}
